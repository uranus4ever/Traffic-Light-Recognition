...
def draw_background_highlight(image, draw_img, width):
    '''
    draw a background on top of the image
    :param image: original image. np format
    :param draw_img: target image (copy of original image)
    :param width: image width
    :return: combination of image with background
    '''
    mask = cv2.rectangle(np.copy(image), (width//2, 0), (width, 155), (0, 0, 0), thickness=cv2.FILLED)

    return cv2.addWeighted(src1=mask, alpha=0.3, src2=draw_img, beta=0.8, gamma=0)


def plot_origin_image(image, image_np, boxes, classes, scores, category_index, red_flag):
    ...
    # draw background
    img_cp = draw_background_highlight(np.copy(image), image_np, im_width)

    # write color and number result
    cv2.putText(img_cp, 'Traffic Lights: ', (im_width//2 + 50, 50),
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2, cv2.LINE_AA)
    if red_flag:
        cv2.putText(img_cp, 'RED / YELLOW', (im_width//2 + 300, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2, cv2.LINE_AA)
    else:
        cv2.putText(img_cp, 'Green / None', (im_width//2 + 300, 50),
    ...
    with detection_graph.as_default():
        with tf.Session(graph=detection_graph) as sess:
            ...
            for image_path in TEST_IMAGE_PATHS:
                ...
                if plot_flag:
                    box_pos = plot_origin_image(image, image_np, boxes, classes, scores, category_index, red_flag)
